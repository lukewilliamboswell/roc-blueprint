...
Sexpr :: [].{
	...
	parse : Str -> Try(a, [InvalidSexpr(Str), ..errs])
		where [a.parser_for : Format -> (List(Token) -> Try({ value : a, rest : List(Token) }, [InvalidSexpr(Str), ..errs]))]
	parse = |text| {
		Shape : a
		tokens =
			match tokenize(text) {
		...
		match parsed.rest {
			[] => Ok(parsed.value)
			[token, ..] => Err(InvalidSexpr("unexpected ${describe(token)} after the value"))
		}
	}
	...
	Format := [Default].{
		...
		encode_list : Out, U64, (Out, (Out, (Out -> Try(Out, err)) -> Try(Out, err)) -> Try(Out, err)) -> Try(Out, err)
		encode_list = |out, _, write_items| {
			finished = write_items(append(out, "("), write_item)?
			Ok(append(finished, ")"))
		}

		encode_tuple : Out, U64, (Out, (Out, (Out -> Try(Out, err)) -> Try(Out, err)) -> Try(Out, err)) -> Try(Out, err)
		encode_tuple = |out, count, write_items| Format.encode_list(out, count, write_items)

		encode_record : Out, U64, (Out, (Out, Str, (Out -> Try(Out, err)) -> Try(Out, err)) -> Try(Out, err)) -> Try(Out, err)
		...
		encode_tag : Out, Str, U64, (Out, (Out, (Out -> Try(Out, err)) -> Try(Out, err)) -> Try(Out, err)) -> Try(Out, err)
				...
				[Symbol(tag), .. as rest] =>
					Encoding.ParseTagUnionSpec.parse(
						spec,
						{
							...
							encoding: format,
							state: rest,
							start_payloads: |state, count| if count == 0 Ok(state) else Err(InvalidSexpr("tag ${tag} needs ${count.to_str()} payload(s); write (${tag} ...)")),
	...
	## Split text into tokens. `;` starts a comment that runs to the end of the line.
	...
	tokenize = |text| {
		...
		while $i < len {
			...
			if byte == '(' {
				...
				is_number = (first >= '0' and first <= '9') or (first == '-' and word != "-")
	...
	is_delimiter = |b| b == '(' or b == ')' or b == ' ' or b == '\n' or b == '\t' or b == '\r' or b == '"' or b == ';'

	read_string : List(U8), U64 -> Try({ value : Str, next : U64 }, [InvalidSexpr(Str)])
...
expect {
	value = { name: "demo", tags: [Plain, WithArgs("a", 2.U64)], nested: { ok: True } }
