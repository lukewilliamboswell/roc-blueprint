...
Project :: [].{
	...
	validate = |ir| {
		...
		Ok(Ir.{ format: ir.format, name: ir.name, requires_: ir.requires_, systems: unique(ir.systems), sources, inputs: ir.inputs, environments: $environments, shells: ir.shells, tasks: ir.tasks, build_sources: ir.build_sources, builds: ir.builds, workflows: ir.workflows, extensions: ir.extensions, raw: ir.raw })
	}
	...
	workflow_graph = |ir| {
		...
		for workflow in ir.workflows {
			$done = visit_workflow(ir.workflows, ir.tasks, ir.builds, workflow.name, [], $done)?
		}
		Ok($done)
	}

	visit_workflow : List(Ir.Workflow), List(Ir.Task), List(Ir.Build), Str, List(Str), List(WorkflowVisit) -> Try(List(WorkflowVisit), Str)
	...
	expand_workflow : List(WorkflowVisit), Str, List(Ir.AtomicStep) -> Try(List(Ir.AtomicStep), Str)
...
build_source_fixture = |build_sources, requires_| {
	...
	Ir.{ format: ir.format, name: ir.name, requires_, systems: ir.systems, sources: ir.sources, inputs: ir.inputs, environments: ir.environments, shells: ir.shells, tasks: ir.tasks, build_sources, builds: ir.builds, workflows: ir.workflows, extensions: ir.extensions, raw: ir.raw }
}
