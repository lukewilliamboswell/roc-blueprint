...
Locks := { identity : LockJson, graph : LockJson }.{
	...
	inputs : Spec -> Try(List(Input), Str)
	inputs = |spec| {
		var $inputs = []
	...
	identity_for : Spec -> Try(LockJson, Str)
	identity_for = |spec| {
	...
	from_nix : Spec, Layout, Str -> Try(Locks, Str)
	from_nix = |spec, layout, text| {
	...
	Spec,
	Layout -> Try(
		{ contents : Str, operations : List(Steps.Operation) },
		Str,
	)
	derive = |locks, spec, layout| {
		Layout.validate(layout)?
		project = Project.validate(spec)?
...
expect match locked_fixture {
	Ok(locks) => {
		project = Project.validate(TestData.project(TestData.data)) ?? Spec.empty("x")
		layout = Layout.{
			project_root: "/moved",
...
expect [
	...
	|(kind, host)| {
		project = TestData.project({
			..TestData.data,
			sources: [{ name: "default", provider: NixPackages("${kind}:NixOS/nixpkgs/nixos-unstable") }],
		})
		...
		Locks.from_nix(project, TestData.layout, native).is_ok()
			and Locks.from_nix(project, TestData.layout, with_host("evil.example")).is_err()
				and match Locks.from_nix(project, TestData.layout, with_host(host)) {
					Ok(locks) => Locks.decode(Locks.encode(locks)) == Ok(locks)
						and Locks.decode(Locks.encode(locks).replace_each(host, "evil.example")).is_err()
					Err(_) => False
				}
	},
)
...
expect [
	...
	|(kind, host)| {
		...
		match (LockJson.decode(with_original_host(host)), LockJson.decode(with_original_host("custom.example"))) {
