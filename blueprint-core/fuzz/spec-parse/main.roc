...
## `Spec.parse` must return a result, never crash or hang, for any text; and
## whatever it accepts must re-encode to text that parses to the same Spec.
##
## The input is raw bytes read as UTF-8, so the seeds in `corpus/` are plain
## Spec text the fuzzer can mutate directly.
	...
	match Str.from_utf8(bytes) {
		...
		Ok(text) =>
			match Spec.parse(text) {
				Ok(spec) => {
					if Spec.parse(spec.to_str()) != Ok(spec) {
						crash "accepted Spec did not survive a re-encode"
					}
					match Project.validate(spec) {
						Ok(project) => {
							if Project.validate(project) != Ok(project) or Spec.parse(project.to_str()) != Ok(project) {
