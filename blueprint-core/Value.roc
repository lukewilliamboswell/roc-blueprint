...
## A generic, JSON-like value, used for provider-specific data in the Spec
...
Value := [Str(Str), Int(I64), Bool(Bool), List(List(Value)), Attrs(List({ name : Str, value : Value }))].{
	is_eq : _

	encoder_for : Format -> (Value, Out -> Try(Out, err))
	encoder_for = |_| |value, out| encode(value, out)

	parser_for : Format -> (List(Token) -> Try({ value : Value, rest : List(Token) }, [InvalidSexpr(Str), ..others]))
	...
	encode = |value, out|
		match value {
			Str(s) => Format.encode_tag(out, "Str", 1, |o, write| write(o, |o2| Format.encode_str(s, o2)))
			Int(n) => Format.encode_tag(out, "Int", 1, |o, write| write(o, |o2| Format.encode_i64(n, o2)))
			Bool(b) => Format.encode_tag(out, "Bool", 1, |o, write| write(o, |o2| Format.encode_bool(b, o2)))
							...
							for attr in attrs {
								$acc = write_item(
									...
									|o4| Format.encode_record(
										...
										2,
										|o5, write_field| {
											named = write_field(o5, "name", |o6| Format.encode_str(attr.name, o6))?
											write_field(named, "value", |o6| encode(attr.value, o6))
										},
									),
								)?
							}
	...
	parse_at : List(Token), U64 -> Try({ value : Value, rest : List(Token) }, [InvalidSexpr(Str), ..others])
	parse_at = |tokens, depth| {
		if depth >= max_depth {
			return Err(InvalidSexpr("value nested deeper than ${max_depth.to_str()} levels"))
		}
		match tokens {
	...
	parse_attr : List(Token), U64 -> Try({ value : { name : Str, value : Value }, rest : List(Token) }, [InvalidSexpr(Str), ..others])
	parse_attr = |tokens, depth| {
		...
		var $more =
			match tokens {
				[Open, .. as rest] => rest
				_ => return Err(InvalidSexpr("expected an attribute ((name ...) (value ...))"))
			}
		while True {
			match $more {
				[Close, .. as after] =>
	...
	close : Value, List(Token) -> Try({ value : Value, rest : List(Token) }, [InvalidSexpr(Str), ..others])
	close = |value, tokens|
		match tokens {
...
sample : Value
sample = Value.Attrs([
	{ name: "x", value: Value.List([Value.Int(-3), Value.Bool(True), Value.Str("s"), Value.List([])]) },
	{ name: "y", value: Value.Attrs([]) },
])

expect Sexpr.parse(Sexpr.to_str(sample)) == Ok(sample)

expect Sexpr.to_str(Value.List([Value.Int(1), Value.Str("a")])) == "(List ((Int 1) (Str \"a\")))"

expect {
	parsed : Try(Value, _)
	parsed = Sexpr.parse("(Attrs (((extra 1) (value (Bool false)) (name \"k\"))))")
